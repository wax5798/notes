...
总共有 4 条匹配结果，只有在 `scripts/Kbuild.include` 的 266 行和 `scripts/Makefile.build` 的 272 行有调用，其余都跟 `fixdep` 调用无关。
...
从 log 可以看到，执行 `make rpi_3_32b_defconfig` 一共调用了 3 次 fixdep，以处理 `scripts/basic/.fixdep.d` 为例，调用的命令的参数分别为：
